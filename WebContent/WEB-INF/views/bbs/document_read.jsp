	...
	<div class="alert alert-danger">
		<h3 style="margin: 0">
			...
			<small>
				...
				<a href="mailto:${readDocument.writerName}">
	...
	<c:if test="${fileList != null}">
				...
				<tr>
					...
					<td>
						<c:forEach var="file" items="${fileList}">
							...
							<!-- 다운로드 링크 -->
							<a href="${downloadUrl}" class="btn btn-link btn-xs"><i class='glyphicon glyphicon-save'></i>${file.originName}
							 </a>
						</c:forEach>
					</td>
				</tr>
		...
		<c:forEach var="file" items="${fileList}">
			<c:if test="${fn:substringBefore(file.contentType, '/') == 'image'}">
				<c:url var="downloadUrl" value="/main/download.do">
					<c:param name="file" value="${file.fileDir}/${file.fileName}" />
				</c:url>
				<p>
